<x-guest-layout>
    ...
    <!-- Session Status -->
    @if(session('status'))
    <div class="mb-4 text-sm text-green-600 bg-green-50 border border-green-200 rounded p-3 text-center">
        {{ session('status') }}
    </div>
    @endif

    <form method="POST" action="{{ $purpose === 'registration' ? route('register.verify') : route('password.otp.verify') }}">
        @csrf

        <div>
            <x-input-label for="code" :value="__('Verification Code')" />
            <x-text-input id="code" class="block mt-1 w-full text-center text-2xl tracking-[0.5em] font-mono"
    ...
    <div class="mt-4 text-center">
        ...
        <form method="POST" action="{{ $purpose === 'registration' ? route('register.verify.resend') : route('password.otp.resend') }}" class="inline">
            @csrf
    ...
    @else
    <div class="mt-3 text-center">
        <a href="{{ route('password.otp.request') }}" class="text-sm text-gray-500 hover:underline">
            Back
        </a>
    </div>
    @endif
